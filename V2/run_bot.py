...
def run_bot_cycle():
    ...
    try:
        ...
        trend_direction_h4 = "UPTREND" if is_uptrend else "DOWNTREND"
        print(f"   Screen 1 (H4): Market is in a {trend_direction_h4}.")
        
        h1_check = df_h1[df_h1.index < current_time]
        if h1_check.empty: raise ValueError("Not enough H1 data for pullback check.")

        pullback_signal = False; trigger_direction = None
        if is_uptrend:
        ...
        if pullback_signal:
            print(f"   Screen 2 (H1): Pullback setup found for a {trigger_direction} trade. Waiting for M15 trigger...")
            ...
            volume_confirm = latest_m15['volume'] > latest_m15['volume_sma_20']
            ...
            if trigger_direction == 'BUY' and ema_cross_up and macd_state_bullish and volume_confirm:
                ...
                message = (f"🔥 *SINYAL BUY BTC/USD*\n\n- *Waktu Sinyal:* `{current_time.strftime('%Y-%m-%d %H:%M:%S')}`\n- *Harga Entry:* `${entry_price:,.2f}`\n\n*Target:*\n- *Take Profit:* `${take_profit:,.2f}`\n- *Stop Loss:* `${stop_loss:,.2f}`")
            ...
            if trigger_direction == 'SELL' and ema_cross_down and macd_state_bearish and volume_confirm:
                entry_price = latest_m15['close']; current_atr = latest_m15['ATR_14']
                stop_loss = entry_price + (2.0 * current_atr); take_profit = entry_price - (4.0 * current_atr)
                message = (f"🚨 *SINYAL SELL BTC/USD*\n\n- *Waktu Sinyal:* `{current_time.strftime('%Y-%m-%d %H:%M:%S')}`\n- *Harga Entry:* `${entry_price:,.2f}`\n\n*Target:*\n- *Take Profit:* `${take_profit:,.2f}`\n- *Stop Loss:* `${stop_loss:,.2f}`")
                send_telegram_notification(message)
                signal_triggered = True
            
            if not signal_triggered:
                print("   Screen 3 (M15): Trigger condition not met in this cycle.")
        else:
            print("   Screen 2 (H1): No pullback setup found.")
